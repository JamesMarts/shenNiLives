...
public class DateUtil {
    ...
    public static String getcurrentTime(long sstime) {
     ...
     * @param exitTime    退出的时间
     * @return 再进来时时间差  毫秒
     */
    public static Long getGrpTime(Long exitTime) {

        return getCurTime()-exitTime;

    }

    public static long getoldTime(int value) {
        return System.currentTimeMillis() - 24 * 60 * 60 * 1000 * value;
    }

    public static String getOldTime(int value) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        // yyyy-MM-dd HH:mm:ss
        String date = sdf.format(new Date(System.currentTimeMillis() - 24 * 60
                * 60 * 1000 * value));
    ...
    // 按照给定的格式化字符串解析日期
    public static java.util.Date parseDate(java.util.Date date, String formatStr)
            throws ParseException {
        String strdate = "";
        SimpleDateFormat sdf = new SimpleDateFormat(formatStr);
